class CountingChannelManager {
	constructor(channel) {
		...
		this.base = null;
		this.by = null;
		this.lastNumber = global.client.data.last_number;
	}
	...
	async init() {
		...
		if (!base)
			this.base = 10;
		else if (global.client.baseMap[base] !== undefined)
			this.base = global.client.baseMap[base];
		else if (!isNaN(base))
		...
		/*if (this.channel.guild.owner && !this.channel.permissionsFor(global.client.user).has('READ_MESSAGE_HISTORY')) return this.channel.guild.owner.send({
			embed: {
				color: 13373206,
		...
		try { // If you uncomment the permission checker comment this line
			...
			lastMessage = (await this.channel.messages.fetch({ limit: 50 }) || []).find((number) => this.parseNumber(number) >= 0 || this.parseNumber(number) <= 0);//this.by.toString().startsWith('-') ? this.parseNumber(number) <= 0 : this.parseNumber(number) >= 0);
	...
	handleNewMessage(bot, message) {
		...
		if (bot.data.debug) {
			...
			console.info("Messups:", bot.messups.get(message.channel.id).get(message.author.id));
		}
		...
		if ((message.content.length != gLastNumber.toString().length && number !== gLastNumber) || !number || !this.isNextInSequence(number) || message.attachments.size || message.embeds.length || RegExp(/([\s\+\=\_\`\~\!\@\#\$\%\^\&\*\(\)\\\|\]\[\{\}\'\"\;\:\?\/\,\<\>\t\r\na-z])/i).test(message.content) || RegExp(/(^(-0+|-{2,}))/i).test(message.content) || (message.content.length >= 2 && RegExp(/^(-?0+)/i).test(message.content)) || (!this.by.toString().startsWith('-') && message.content.includes('-')) || (this.by.toString().startsWith('-') && message.content.startsWith('-') && message.content.split('').filter((char) => char === '-').splice(1).length >= 1) || (!this.by.toString().includes('.') && message.content.includes('.')) || (this.by.toString().includes('.') && message.content.includes('.') && message.content.split('').filter((char) => char === '.').splice(1).length >= 1)) {
			bot.emit('messup', message);
			if (bot.messups.has(message.channel.id) && bot.messups.get(message.channel.id).has(message.author.id) && bot.messups.get(message.channel.id).get(message.author.id) >= bot.data.allowed_messups) bot.emit('giveMemberCantCount', message);
			return bot.emit('handleDelete', message);
		}
		//if (!number) return bot.emit('handleDelete', message, 'user');
		//if (!this.isNextInSequence(number)) return bot.emit('handleDelete', message, 'user');
