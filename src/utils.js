...
const get_instances = async (message, search, options = { search_blocked: false, show_hidden: true, get_types: [], guild_only: [], channel_types: [] }) => {
	...
	if (options.search_blocked) {
		const { guild: { settings: { countingChannels }, channels, members, roles } } = message;
		const set_channels = new Collection();
		set_channels.set('channels', new Collection());

		if (countingChannels && countingChannels.length) countingChannels.map((channel_id) => set_channels.get('channels').set(channel_id, channels.has(channel_id) ? channels.get(channel_id) : `${channel_id} (Invalid)`));
		if (RegExp(['all','channel','channels'].join('|')).test(options.get_types.join(' '))) set_channels.get('channels').filter((channel) => (options.channel_types && options.channel_types.length ? options.channel_types : ['voice','text','category']).includes(channel.type) && (channel === search || channel.toString() === search || channel.id === search || channel.name.toLowerCase().includes(search.toLowerCase()))).map((channel) => items.push(channel));
	} else {
